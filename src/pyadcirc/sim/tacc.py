...
TACC_SYSTEMS = {'ls6': {'queue':'normal',
    'queue_test':'development'},
    'frontera':{'queue':'normal',
        'queue_test':'development'}, 'stampede2':{'queue':'skx-normal',
    'queue_test':'skx-dev'}}
...
def init_system(jm_id, system, user=None, psw=None, mfa=None, test=True):
    ...
    for app_dir in (Path(__file__).parent / 'hpc_apps').iterdir():
        if app_dir.name in list(APPS.keys()):
            ...
            _ = tjm.deploy_app(jm_id, app_config=app_config,
...
class ADCIRCDB(object):
    ...
    def __init__(self,
        ...
        base_contents = tjm.list_files(self._jm['jm_id'],
                str(self.base_adcirc_dir.parent))
        if 'adcirc' not in [f['filename'] for f in base_contents]:
            local_adcirc_dir = Path(__file__).parents[3] / 'data/adcirc'
            tjm.upload(self._jm['jm_id'], local_adcirc_dir,
                    self.base_adcirc_dir)
    ...
    def list_ADCIRC_inputs(self):
        ...
        input_files = tjm.list_files(self._jm['jm_id'],
                self.base_adcirc_dir / 'inputs')
        return input_files

    def list_ADCIRC_execs(self):
        """List ADCIRC exec files configured"""
        input_files = tjm.list_files(self._jm['jm_id'],
                self.base_adcirc_dir / 'execs')
        return input_files

    def compile_ADCIRC(self,
            version='v55.01',
    ...
    def list_ncar_data(self,
        ...
        res = filter_res(db.dt.list_files(ds, start_date, end_date, pp=False),
                         search=search, match=match, print_res=print_res)

        return res

    def init_ncar_data_transfer(self,
                                ds_name,
                                fields,
                                start_date,
                                end_date):
        ...
        tdata = self.dt.stage(ds_name, fields, GLOBUS_ENDPOINT, str(dest_path),
                              start_date, end_date)
        tresults = self.dt.submit(tdata)
        self.data_transfers.append((tdata, tresults))

    def check_ncar_data_transfer(self,
        ...
        return filter_res(status_info, fields=fields, search=search, match=match,
                          print_res=print_res)
...
class ADCIRCSim(object):
    ...
    def deploy_job(self,
        ...
        job_config = {
                ...
                'max_run_time': rt,
                'inputs': {},
                'parameters': {'inputDirectory': self.input_dir,
    ...
    def submit_job(self):
        """Get root dir shared accross all TACC work systems"""
        self.job_config = tjm.submit_job(self._jm['jm_id'],
